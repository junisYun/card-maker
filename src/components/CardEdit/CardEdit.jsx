...
const CardEdit = ({ card, updateCard, deleteCard }) => {
  const { id, name, company, theme, professional, email, message } = card;
  ...
  return (
    <form className={styles.form} key={id} data-id={id}>
      ...
      <input className={styles.input} ref={professionalRef} type="text" name="professional" value={professional} onChange={onChange} />
      <input className={styles.input} ref={emailRef} type="text" name="email" value={email} onChange={onChange} />
      <textarea className={styles.textarea} ref={messageRef} name="message" value={message} onChange={onChange}></textarea>
      <div className={styles.fileInput} onClick={onSubmit}>
        <ImageFileInput />
      </div>
